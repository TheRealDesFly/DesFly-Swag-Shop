...
3. Staging monitoring no longer reports skipped work as live success.
   ...
   - Live staging probes are default-branch only, run inside 09:00-23:00 MYT, use read-only repository permissions, serialize live overlap separately from offline validation, and have timeouts.
...
- Wix inspection found twelve of seventeen required runtime backend secret names after the production credential set was added. The remaining missing names are `ISTORE_ISEND_DEPLOYED_REVISION`, `ISTORE_ISEND_WEBHOOK_SECRET`, `ISTORE_ISEND_SINGLE_PARCEL_CONTRACT_CONFIRMED`, `ISEND_FULFILLMENT_TRIGGER_SECRET`, and `ISEND_RECOVERY_TRIGGER_SECRET`.
...
2. Verify all seventeen Wix backend secret names in `STAGING_SETUP.md` without exporting their values. Provision the five currently missing Wix names, and bind `ISTORE_ISEND_DEPLOYED_REVISION` to the exact reviewed source SHA. Keep `ISTORE_ISEND_SINGLE_PARCEL_CONTRACT_CONFIRMED` unset or non-`true` until the named iSend approver confirms the complete single-parcel contract.

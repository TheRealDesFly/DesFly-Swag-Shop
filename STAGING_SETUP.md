...
- `ISendOrderOutboxClaims` stores append-only, generation-fenced worker leases. Add a compound index on (`claimKey`, `generation`) and regular indexes on `leaseExpiresAt` and `releasedAt`.
...
| `ISendMaintenanceState` | `lastRunDurationMs`, `lastRunScanned`, `lastRunDeleted`, `lastRunPreservedInvalid`, `lastRunPreservedUnverified`, `lastRunStaleUnreleased`, `lastRunEligibleDeferred`, `claimItemLimit`, `measuredUniqueClaimKeysPerDay`, `lifecycleIntentItemLimit`, `measuredLifecycleIntentRowsPerDay`, `webhookPayloadRetentionDays`, `sentEmailRetentionDays` | Number | Retention/capacity measurements and approved retention periods |
| `ISendMaintenanceState` | `lastRunAttentionRequired`, `lastRunThrottled`, `lastRunRuntimeLimited`, `lastRunScanTruncated`, `sensitiveDataRetentionPolicyApproved`, `webhookPayloadRetentionEnforcedExternally`, `sentEmailRetentionEnforcedExternally` | Boolean | Fail-closed retention, capacity, and PII-policy gates |
| `ISendMaintenanceState` | `lastRunAttentionReasons`, `capacityEvidenceProvenance`, `sensitiveDataRetentionPolicyProvenance` | Object | Bounded reasons and retained provenance |
...
Raw `ISendWebhookEvents.payload` records and sent `ISendPendingEmails` contain sensitive data. No deletion interval is hard-coded because the owner must approve the retention periods and enforcement mechanism. Record those periods, policy revision, recent verification, and provenance in `ISendMaintenanceState`; operational health remains red unless external enforcement for both collections is explicitly attested. Never delete unsent email, ambiguous fulfillment, or other active evidence merely to satisfy retention.

...
- `ISendOrderOutbox` stores the complete Wix order snapshot and queue/lifecycle state. Wix allows three regular custom indexes on this collection, so use compound indexes on (`status`, `environment`, `nextAttemptAt`) and (`status`, `environment`, `leaseExpiresAt`), plus a regular `lifecycleRequiresAttention` index. The retry-exhausted health query must be scoped to the current environment and uses the status/environment prefix of the queue indexes; mismatch and unassigned scans cover other environment states. Its deterministic item ID supplies the order-key identity boundary.
